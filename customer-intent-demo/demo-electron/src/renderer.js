...
const insecureInput = document.getElementById("insecure-tls");
...
async function routeCase(caseItem) {
  ...
  const result = await window.demoApi.routeRequest({
    url: backendUrl,
    text: caseItem.text,
...
insecureInput.addEventListener("change", () => {
  insecureTls = insecureInput.checked;
});

(async function init() {
  backendUrl = await window.demoApi.getDefaultBackend();
